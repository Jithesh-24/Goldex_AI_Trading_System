#!/usr/bin/env python3
"""
add_renaissance_to_mmap_v2.py — OOM-safe Renaissance feature recomputation.
Uses memmap slices (never copies full matrix to RAM).
Computes HMM, Kalman, OU, GARCH, correlation correctly.
"""
import numpy as np
...
TAIL = 10_000_000
STRIDE = 50  # compute every 50th row
...
def main():
    ...
    # Copy base features from old mmap (OOM-safe: row by row in chunks)
    print(f"Copying {nf_old} base features (OOM-safe)...", flush=True)
    ...
    for i in range(start, start + n_tail, CHUNK):
        ...
        chunk = np.array(X_full[i:end])  # small chunks fit in RAM
        X_new[copied:copied + len(chunk), :nf_old] = chunk
        copied += len(chunk)
        if copied % 500_000 == 0:
            print(f"  Copied {copied:,}/{n_tail:,} rows", flush=True)
    del X_full
    X_new.flush()
    print(f"  ✅ Base features copied ({time.time()-t0:.0f}s)", flush=True)
    ...
    for i in range(lookback, n_tail, STRIDE):
        ...
        # Kalman (filter_series — correct API)
        try:
