...
# ── v8.9 TICK-FLOW STATE (2026-08-12): live equivalents of the Dukascopy
# tick features the next retrain learns. The training matrix computes, per
# M5 bar: vol_rel = window volume / rolling-288 mean; cvd = decaying
# cumulative flow. Live, xm_ticker only publishes ticks_300s / imb_300s —
# the engine accumulates them here into the SAME definitions so that once
# features.json lists them, X build feeds REAL values, not 0.0.
_TICK_HIST = {"tick_vals": [], "imb_vals": [], "n": 0}

def tick_flow_state(ms, max_hist=600):
    """Accumulate ticks_300s / imb_300s from the ticker state; return
    vol_rel + cvd mirroring build_tick_matrix.py's exact formulas."""
    global _TICK_HIST
    ticks = float(ms.get("ticks_300s", 0.0))
    imb = float(ms.get("imb_300s", 0.0))
    h = _TICK_HIST
    h["tick_vals"].append(ticks)
    h["imb_vals"].append(imb)
    h["n"] += 1
    if len(h["tick_vals"]) > max_hist:
        h["tick_vals"] = h["tick_vals"][-max_hist:]
        h["imb_vals"] = h["imb_vals"][-max_hist:]
    # vol_rel: window volume / rolling mean (train uses 288-bar window; live
    # uses available history, expanding mean until 288 accumulated)
    hist = h["tick_vals"]
    win = hist[-288:] if len(hist) >= 288 else hist
    mean_ticks = sum(win) / max(len(win), 1)
    vol_rel = ticks / max(mean_ticks, 1e-9)
    # cvd: decaying cumulative flow — same decay as the training matrix
    n = len(h["imb_vals"])
    cvd = 0.0
    for i, v in enumerate(h["imb_vals"]):
        cvd += v * (0.999 ** (n - 1 - i))
    return {"vol_rel": vol_rel, "cvd": cvd}
...
def main():
    ...
    # ── v8.9 CLOSE ANCHOR (2026-08-11) ──
    # The cooldown used _last_signal["time"] = the trade OPEN. A trade that
    # ran minutes made the 30s floor already-passed at the moment of close,
    # so a new signal re-fired 2 SECONDS after every close — on the SAME
    # cached M5 feature vector (fx cache is keyed per bar) → the same
    # analysis on identical inputs, flipping direction into losers
    # (journal: 16:52:29 TP→16:52:31 BUY→SL; 17:11:28 SL→17:11:30 SELL→SL;
    # 17:15:52 SL→17:15:54 SELL→SL). Record the close (time + M5 bar + dir +
    # px) at every close path so the suppression block can anchor to it.
    # State-based: a same-bar re-fire is the SAME bet, held until the bar
    # completes (fresh features) or price really moves (>=0.5xATR). No
    # timers, no hardcoded avoidance — the market state decides.
    def _note_close(direction, px_close):
        try:
            _m5 = int(fc.times[-1] // 300) * 300 if len(fc.times) else None
        except Exception:
            _m5 = None
        globals()["_last_close"] = {"time": time.time(), "m5": _m5,
                                    "dir_buy": (direction == "BUY"),
                                    "px": float(px_close),
                                    "regime": active.get("regime", ""),
                                    "tp": abs(float(active.get("tp", 0.0)) - float(active.get("entry", 0.0)))}
    ...
    if active:
        ...
        if st:
            ...
            if verdict in ("SL", "TP") and not partial:
                ...
                _note_close(d0, active["sl"] if verdict == "SL" else active["tp"])
                ...
                if resolved:
                    ...
                    _note_close(d0, active["sl"] if r_kind == "SL" else active["tp"])
                    active = None; save_active(None)
                elif partial:
                    ...
                    if beyond:
                        ...
                        _note_close(d0, bid0 if d0 == "BUY" else ask0)
                        active = None; save_active(None)
                    else:
    ...
    def maybe_reload_model():
        ...
        try:
            ...
            if model_mtime is not None and max(m, dm) > model_mtime:
                ...
                # v8.8 (2026-08-11): if the new ensemble was trained on a feature
                # list this features.json hasn't caught up to (train_ai saves the
                # model first, features.json second — microsecond window), feeding
                # mismatched dims would throw a LightGBM dimension error. Refuse
                # the reload and keep the last valid pair. NOTE: do NOT advance
                # model_mtime here — leave it stale so the very next loop cycle
                # retries the reload and picks the pair up the moment features.json
                # lands (engine keeps trading on the old consistent pair meanwhile).
                if models and models[0].num_feature() != len(feats):
                    print(f"[{ts()}] ⚠️ reload: ensemble({models[0].num_feature()} feats) != features.json({len(feats)}) — keeping last valid pair until consistent")
                    return
                try:
                    with open(DIR_FEATURES) as f:
    ...
    while True:
        try:
            ...
            if active:
                ...
                if st and st.get("verdict") in ("SL", "TP") and not st.get("coverage_partial"):
                    ...
                    append_journal({"t": time.time(), "dir": d, "entry": active["entry"],
                                    ...
                                    "src": "ticker_partial"})
                    _note_close(d, bid if d == "BUY" else ask)
                ...
                if hit:
                    ...
                    try:
                        ...
                        pass
                    # ── JANE STREET: RECORD TRADE FOR CIRCUIT BREAKER ──
                    try:
                        from jane_street_additions import record_trade_result, load_pnl_state
                        _pnl_st = load_pnl_state()
                        record_trade_result(_pnl_st, pnl)
                    except Exception:
                        pass
                    if r_kind == "TIME":
                        emoji = "⏰"
                    ...
                    _note_close(d, active["sl"] if r_kind == "SL" else active["tp"])
                    active = None; save_active(None); continue
            ...
            if fx is None:
                time.sleep(poll); continue
            # v8.8 (2026-08-11): inject position-state into the LIVE feature
            # vector BEFORE the sweep. The matrix now carries day_pnl / streak /
            # trades_today as training features (0.0 baseline on historical
            # bars; real values on live outcome rows via the closed loop).
            # Feeding the real state at prediction time lets the model LEARN
            # "after N losses the next trade's P(win) shifts" — a learned
            # re-pricing of the same setups, NOT a gate that cuts trade
            # frequency. Harmless pre-retrain (features.json doesn't list them
            # yet → X build ignores the keys).
            try:
                fx = dict(fx)
                fx["day_pnl"] = float(day_pnl_engine)
                fx["streak"] = float(streak_engine)
                fx["trades_today"] = float(trades_today_engine)
            except Exception:
                pass
            ...
            if st:
                ...
                xm_spread = 0.20
            # v8.9 (2026-08-11): LIVE TICK MICROSTRUCTURE into the decision
            # vector — user directive: decide on TICK calculation, not just
            # closed M5 bars. xm_ticker.py polls real XM ticks at 25ms and
            # publishes ticks_60s/300s, tick imbalance, spread dynamics in
            # xm_tick_state.json[ms]. Inject them into fx so the placement
            # sweep re-prices setups by live flow (delta, activity, spread
            # stress). Zero-lag: these are the PRESENT, not a closed bar.
            # Harmless pre-retrain (not in features.json → X build ignores);
            # the closed-loop outcome rows already carry them → next retrain
            # teaches "trade WITH the tick flow" from data, never a gate.
            try:
                if st:
                    _ms = st.get("ms") or {}
                    fx["ticks_60s"] = float(_ms.get("ticks_60s", 0))
                    fx["ticks_300s"] = float(_ms.get("ticks_300s", 0))
                    fx["imb_60s"] = float(_ms.get("imb_60s", 0))
                    fx["imb_300s"] = float(_ms.get("imb_300s", 0))
                    _s_now = float(_ms.get("spread_now", 0.2))
                    _s_mean = float(_ms.get("spread_mean_60s", 0.2))
                    _s_std = float(_ms.get("spread_std_60s", 1e-6))
                    fx["spread_z60"] = (_s_now - _s_mean) / max(_s_std, 1e-6)
                    fx["spread_now"] = _s_now
                    # v8.9 TICK-FLOW (2026-08-12): scale-free flow features the
                    # tick retrain learns — fed live with the SAME definitions.
                    _tf = tick_flow_state(_ms)
                    fx["vol_rel"] = _tf["vol_rel"]
                    fx["cvd"] = _tf["cvd"]
            except Exception:
                pass
            # ── v8.9c ADVANCED QUANT FEATURES (2026-08-13):
            # Compute Hurst, Amihud, Kyle lambda, VPIN from live price history
            # These features identify: trending vs mean-reverting, price impact,
            # informed trading, return persistence — all proven by research.
            try:
                if len(price_history) >= 50:
                    _ph = np.array(price_history[-200:])  # last 200 prices
                    _rh = np.diff(_ph) / _ph[:-1] if len(_ph) > 1 else np.array([0.0])
                    _vh = np.ones(len(_rh))  # unit volume for live
                    
                    from advanced_features import (hurst_exponent, amihud_illiquidity,
                                                   kyle_lambda, vpin_estimate,
                                                   return_autocorrelation, variance_ratio,
                                                   entropy)
                    fx["hurst"] = hurst_exponent(_ph)
                    fx["amihud"] = amihud_illiquidity(_rh, _vh)
                    fx["kyle_lambda"] = kyle_lambda(_rh, _vh)
                    fx["vpin"] = vpin_estimate(_vh, _rh)
                    fx["return_autocorr"] = return_autocorrelation(_rh)
                    fx["variance_ratio"] = variance_ratio(_rh)
                    fx["entropy"] = entropy(_rh)
                else:
                    for _af in ["hurst", "amihud", "kyle_lambda", "vpin",
                                "return_autocorr", "variance_ratio", "entropy"]:
                        fx[_af] = 0.5 if _af == "variance_ratio" else 0.0
            except Exception:
                pass
            # ── REGIME ROUTER (v7.7, 2026-08-04) ──
            # User decision: macro data (DXY, yields) is 1-day lagged from
            # Yahoo Finance. For real-time gold trading, pure gold price
            # action is more valuable. The system now learns from 122
            # gold-intrinsic features only (microstructure, technicals,
            # session, events, patterns).
            # Uncomment below to re-enable macro injection:
            # try:
            #     with open(f"{OUTDIR}/macro_state.json") as _mf:
            #         _mac = json.load(_mf)
            #     if time.time() - _mac.get("ts", 0) < 3600:
            #         for _k in ("dxy_z", "dxy_5d_chg", "tnx_level", "tnx_5d_chg",
            #                    "gc_5d_chg", "gld_5d_chg", "eur_5d_chg"):
            #             fx[_k] = float(_mac.get(_k, 0.0))
            # except Exception:
            #     pass
            # ── v8.9c RENAISSANCE MODULES (2026-08-13):
            # HMM regime detection, Kalman filter, OU mean reversion, Half-Kelly
            # Inspired by Jim Simons / Renaissance Technologies approach
            try:
                if len(price_history) >= 100:
                    _rp = np.array(price_history[-200:])
                    _rr = np.diff(_rp) / _rp[:-1] if len(_rp) > 1 else np.array([0.0])
                    
                    from renaissance_modules import (HMMRegimeDetector, KalmanFilter,
                    ...
                    # HMM Regime Detection (Leonard Baum's algorithm)
                    _hmm = HMMRegimeDetector(n_states=4, n_iter=10)
                    if len(_rr) >= 150:
                        _hmm.fit(_rr[-150:])
                        _hmm_regime, _hmm_probs = _hmm.get_current_regime(_rr, window=50)
                        fx['hmm_regime'] = float(_hmm_regime)
                        for _i in range(4):
                            fx[f'hmm_prob_{_i}'] = float(_hmm_probs[_i])
                    
                    # Kalman Filter (signal extraction from noise)
                    _kf = KalmanFilter(dt=1.0, process_noise=0.01, measurement_noise=0.1)
                    _kf_trends, _kf_innov = _kf.filter_series(_rp[-100:])
                    fx['kalman_trend'] = float(_kf_trends[-1, 0])
                    fx['kalman_velocity'] = float(_kf_trends[-1, 1])
                    fx['kalman_innovation'] = float(_kf_innov[-1])
                    
                    # Ornstein-Uhlenbeck (mean reversion detection)
                    _ou = OrnsteinUhlenbeckDetector(lookback=100)
                    _ou_result = _ou.fit(_rp)
                    fx['ou_theta'] = float(_ou_result['theta'])
                    fx['ou_mu'] = float(_ou_result['mu'])
                    fx['ou_half_life'] = min(float(_ou_result['half_life']), 999.0)
                    fx['ou_is_mr'] = 1.0 if _ou_result['is_mean_reverting'] else 0.0
                    fx['ou_signal'] = float(_ou_result['signal'])
                    fx['ou_z_score'] = float(_ou_result.get('z_score', 0))
                    
                    # GARCH Volatility (EGARCH asymmetric vol model)
                    _garch = compute_garch_features(_rr[-200:], window=200)
                    fx['garch_vol'] = _garch['garch_vol']
                    fx['garch_forecast'] = _garch['garch_forecast']
                    fx['vol_regime'] = _garch['vol_regime']
                    fx['vol_persistence'] = _garch['vol_persistence']
                    fx['vol_asymmetry'] = _garch['vol_asymmetry']
                    fx['vol_shock'] = _garch['vol_shock']
                    
                    # Correlation Breakdown Detection
                    _corr = compute_correlation_features(_rr[-200:], window=20)
                    fx['corr_dxy'] = _corr['corr_dxy']
                    fx['corr_vix'] = _corr.get('corr_vix', 0.0)
                    fx['corr_change_dxy'] = _corr['corr_change_dxy']
                    fx['corr_regime'] = _corr['corr_regime']
            except Exception:
                pass
            # ── REGIME ROUTER (v7.7, 2026-08-04) ──
            ...
            _lc = globals().get("_last_close")
            _sup = False
            # v8.9 (2026-08-11): the cooldown is anchored to the most recent
            # of (signal fire, trade CLOSE). Previously only the signal fire
            # was used — a trade that ran 20 min made the 30s floor already
            # passed at the moment of close, so a new signal re-fired 2s after
            # every close (journal: 16:52:29 TP → 16:52:31 BUY → SL; 17:11:28
            # SL → 17:11:30 SELL → SL; 17:15:52 SL → 17:15:54 SELL → SL).
            # The fix is STATE-based, not a clock: fx is cached per M5 bar, so
            # a signal on the SAME bar as the last close is the SAME analysis
            # on IDENTICAL inputs — not a new opportunity. It is held until
            # the next bar completes (fresh features) or price makes a real
            # move (>=0.5xATR). The winning pattern (16:31 close → 16:35
            # signal, 3m56s) crossed a bar boundary and is unaffected.
            _ev_t = None
            if _last is not None:
                _ev_t = _last["time"]
            if _lc is not None and (_ev_t is None or _lc["time"] > _ev_t):
                _ev_t = _lc["time"]
            if _ev_t is not None:
                _tago = time.time() - _ev_t
                _is_buy_now = (buy[4] >= sell[4]) if (buy and sell) else (buy is not None)
                if _tago < MIN_SIGNAL_GAP:
                    _sup = True
                elif _lc is not None and _lc.get("m5") is not None:
                    # SAME-BAR HOLD: last close happened on this M5 bar → the
                    # candidate signal uses the identical cached feature vector
                    # → same bet, not a new market state.
                    try:
                        _cur_m5 = int(fc.times[-1] // 300) * 300
                    except Exception:
                        _cur_m5 = None
                    _moved_c = abs(px - _lc.get("px", px))
                    if _cur_m5 is not None and _cur_m5 == _lc["m5"] and _moved_c < SAME_IDEA_MOVE * atr:
                        _sup = True
                if not _sup and _last is not None:
                    _same_dir = (_is_buy_now == _last.get("dir_buy"))
                    if _same_dir and _last.get("regime") == route_regime:
                        ...
                        if _moved < SAME_IDEA_MOVE * atr and _best is not None and abs(_best[1] - _last.get("tp", -1)) <= SAME_IDEA_BAND:
                            _sup = True
            if _sup:
                _rpx = (_lc or _last).get("px", px)
                print(f"[{ts()}] ⏸ hold same-idea (state unchanged {time.time()-_ev_t:.0f}s, regime {route_regime}, Δ{abs(px-_rpx):.2f}) — waiting for a REAL new move")
                time.sleep(poll); continue
            ...
            if direction is not None and exp > 0:
                # ── v8.9c CIRCUIT BREAKER REMOVED (2026-08-13):
                # User decision: system must NEVER stop trading.
                # Instead of circuit breaker, the system self-rectifies:
                # - Daily learning analyzes every trade
                # - Retrain triggers on poor performance
                # - Model adjusts weights automatically
                # - No hardcoded stops — pure learning from losses
                pass
                # ── JANE STREET: COST-AWARE GATE (v8.9) ──
                # Don't trade if expected alpha < total transaction cost
                try:
                    from jane_street_additions import cost_aware_gate, TOTAL_COST_USD
                    if not cost_aware_gate(exp, sl_dist, atr):
                        print(f"[{ts()}] 💰 cost gate: exp=${exp*sl_dist:.2f} < cost=${TOTAL_COST_USD*2:.2f} ({direction})")
                        time.sleep(poll); continue
                except Exception:
                    pass
                ...
                # ── JANE STREET: FEATURE DRIFT CHECK (v8.9) ──
                # Monitor live feature distributions vs training.
                # Don't block, but penalize confidence on drift.
                try:
