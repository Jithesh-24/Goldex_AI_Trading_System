...
STEPS = [
    ...
    ("generate_drift_stats.py",     "feature_drift_stats.json", "drift detection stats"),
]
...
def run_step(script, artifact, label, skip_specialists=False):
    ...
    env = dict(os.environ)
    # v8.9c: PURE GOLD training — macro columns skipped at read time
    # train_ai.py FEATURE_EXCLUDE now includes dxy/tnx/gc/gld/eur
    tick_csv = f"{BASE}/gold_features_m5_full.csv"
    m5_csv = f"{BASE}/gold_features_m5.csv"
    env["FEAT_CSV"] = tick_csv if os.path.exists(tick_csv) else m5_csv
    print(f"  Matrix: {env['FEAT_CSV']}", flush=True)
    env["PRIOR_BAR_SECS"] = "300"        # M5 bars = 300s
    env["PRIOR_HORIZONS"] = "3"  # v8.8: trade key must be h3 (15-min), not h36    # 15/60/180 min at M5
