"""Confirm: mismatch fraction confined to SL 3.4/4.5 groups (rotated/mislabeled
in the SOURCE matrix); SL 0.8-2.6 groups match exactly."""
import sys
import numpy as np
import pandas as pd
...
mm_by_sl = defaultdict(lambda: [0, 0])   # sl_mult -> [mismatch, total]
for bi in range(n):
    bt = p0["time"].iloc[bi]
    bar = src[src["time"] == bt]
    sdict = {}
    ...
    for gi in range(n_old):
        ...
        if not ok:
            mm_by_sl[m][0] += 1
print("mismatch by SL group (order-agnostic, full period 0):")
for m in F.SL_MULTS:
    mm, tot = mm_by_sl[m]
    print(f"  SL {m}: {mm}/{tot} mismatched ({100*mm/tot:.1f}%)")
