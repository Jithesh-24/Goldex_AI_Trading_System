"""Re-run per-geom mismatch with realistic tolerance (1% relative on mfe/mfa,
exact on target) to isolate REAL mismatches from float32 rounding."""
import sys
import numpy as np
import pandas as pd
...
mm_target = defaultdict(lambda: [0, 0])
for bi in range(n):
    bt = p0["time"].iloc[bi]
    bar = src[src["time"] == bt]
    sdict = {}
    ...
    for gi in range(n_old):
        ...
        mm_target[(m, r, direction)][1] += 1
        if k not in sdict:
            mm[(m, r, direction)][0] += 1
            mm_target[(m, r, direction)][0] += 1
            continue
        sr = sdict[k]
        ...
        if not ok:
            mm[(m, r, direction)][0] += 1
tot_t = sum(v[0] for v in mm_target.values()); tot_all = sum(v[1] for v in mm_target.values())
tot_m = sum(v[0] for v in mm.values())
print(f"TARGET-only mismatches: {tot_t}/{tot_all} ({100*tot_t/tot_all:.1f}%)")
print(f"full (tgt+mfe+mfa @1%) mismatches: {tot_m}/{tot_all} ({100*tot_m/tot_all:.1f}%)")
print("\nworst groups by target mismatch:")
for key, (mc, tc) in sorted(mm_target.items(), key=lambda x: -x[1][0])[:6]:
    print(f"  m={key[0]} r={key[1]} {key[2]:4s}: {mc}/{tc}")
print("\nbest groups by target mismatch:")
for key, (mc, tc) in sorted(mm_target.items(), key=lambda x: x[1][0])[:6]:
    print(f"  m={key[0]} r={key[1]} {key[2]:4s}: {mc}/{tc}")
