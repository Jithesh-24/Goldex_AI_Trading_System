...
_m5 = f"{BASE}/gold_features_m5.csv"
...
def main():
    ...
    # Quick matrix row count (first chunk tells us rows per chunk, then count chunks)
    first_chunk = pd.read_csv(MATRIX, nrows=1)
    chunk_size = len(first_chunk)
    # For alignment: OOF must match matrix rows. If specialist OOF is smaller,
    # it was built from a filtered subset and won't align with full matrix chunks.
    # Check: does specialist OOF size divide evenly into expected matrix size?
    # Better: just try specialist, catch the size mismatch early.
    if os.path.exists(spec_full):
        oof_spec = np.load(spec_full)
        oofy_spec = np.load(f"{MODEL_DIR}/oof_spec_full_y.npy")
        # Heuristic: specialist OOF should be ~same size as matrix (±0.1%)
        # If it's off by >1000 rows, the chunk alignment will fail
        expected_rows = 32_509_156  # from tick matrix build output; safe fallback
        if abs(len(oof_spec) - expected_rows) < 10_000:
            oof = oof_spec
            oofy = oofy_spec
            print(f"OOF source: SPECIALIST full-matrix ({len(oof):,} rows) — engine parity")
            USE_SPEC_CALS = True
        else:
            print(f"⚠️ Specialist OOF size mismatch: {len(oof_spec):,} vs expected ~{expected_rows:,}")
            print(f"   Falling back to base ensemble OOF")
