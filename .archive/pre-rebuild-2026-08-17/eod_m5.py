...
def main():
    ...
    steps = [
        ...
        # v8.9 TICK CLOSED-LOOP (2026-08-12): refresh the Dukascopy tick block
        # for the last 10 days BEFORE the incremental matrix build — new bars
        # must carry imb_300s/vol_rel/cvd or the tick merge finds nothing and
        # the matrix stops growing (reindex→dropna drops tick-less rows).
        (["fetch_dukascopy_m1.py", "--recent-days", "10"], 3600, "Dukascopy tick refresh (10d)"),
        # v8.9b MACRO CLOSED-LOOP (2026-08-12): refresh cross-asset context
        # (DXY/yields/futures/GLD/EURUSD) daily so training macro features stay
        # current — cheap (~25s, Yahoo daily closes).
        (["fetch_macro_context.py"], 600, "cross-asset macro refresh"),
...
"""], capture_output=True, text=True, env=env, timeout=300)
    ...
    # v8.8 FIX (2026-08-11): PRIOR_HORIZONS=3 ONLY — regenerate_dir_prior uses
    # HORIZONS[-1] as the trade key. "3,12,36" wrote the 180-min h36 secular
    # prior (P(up)=0.76) into P_up_by_regime and silently re-broke the h3 fix.
    env["PRIOR_HORIZONS"] = "3"
