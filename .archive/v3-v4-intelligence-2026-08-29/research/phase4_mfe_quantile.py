...
def run_mfe_quantile_candidate_v3b(max_holding: int, rows: int = None, registry_dir: str = None, schemas_dir: str = None) -> dict:
    ...
    oof_pred_with_p = np.full(len(mfe_R), np.nan)
    for _, (train_pos, test_pos) in enumerate(cv.split(t0_s.to_numpy(), t1_s.to_numpy())):
        model = fit_quantile(X_meta_with_p, mfe_R, train_pos, 0.75)
        oof_pred_with_p[test_pos] = model.predict(X_meta_with_p.iloc[test_pos])
    has_pred_with_p = np.isfinite(oof_pred_with_p)
    yp_with_p, yt_with_p = oof_pred_with_p[has_pred_with_p], mfe_R[has_pred_with_p]
    ...
    oof_pred_no_p = np.full(len(mfe_R), np.nan)
    for _, (train_pos, test_pos) in enumerate(cv.split(t0_s.to_numpy(), t1_s.to_numpy())):
        model = fit_quantile(X_meta_no_p, mfe_R, train_pos, 0.75)
        oof_pred_no_p[test_pos] = model.predict(X_meta_no_p.iloc[test_pos])
    has_pred_no_p = np.isfinite(oof_pred_no_p)
    yp_no_p, yt_no_p = oof_pred_no_p[has_pred_no_p], mfe_R[has_pred_no_p]
    ...
    schema = build_schema(f"mfe_quantile_v3b_h{max_holding}", "2026-08-24", feature_cols_final)
    save_schema(schema, schemas_dir=schemas_dir if schemas_dir else SCHEMAS_DIR)
    entry = ModelRegistryEntry(
        model_id=f"mfe_quantile_v3b_candidate_h{max_holding}", family="mfe_quantile", algorithm="catboost_quantile",
        artifact_path=f"registry/mfe_quantile_v3b_candidate_h{max_holding}.json",
        feature_schema_version=f"{schema.schema_id}__{schema.schema_version}",
        feature_cols=feature_cols_final,
        target_definition=(f"mfe_R: max favourable excursion in R-multiples up to t1, direction_side/assumed_side "
    ...
    print(f"[mfe_quantile_v3b h={max_holding}] n_events={len(mfe_R):,} "
