...
R-multiples), V3 feature fabric, using the same fit_quantile/pinball_loss
methodology research/v3_quantile_models.py already applied on the old v2
dataset, reused verbatim here on the new V3-feature event set.
NOTE: unlike Direction/Opportunity, this role does NOT implement a real
spec-section-17 baseline-beats gate -- there is no comparison here against
a per-vol-state empirical-quantile baseline; that comparison is deferred/
not implemented. Global AND per-regime coverage reported (spec
...
def run_mae_quantile_candidate(max_holding: int, rows: int = None, registry_dir: str = None, schemas_dir: str = None) -> dict:
...
def run_mae_quantile_candidate_v3b(max_holding: int, rows: int = None, registry_dir: str = None, schemas_dir: str = None) -> dict:
    ...
    # Pass 1 (q=0.5 only): full pool + assumed_side + p_direction, capture per-fold quantile-model
    # feature importances to narrow to this role's OWN schema before the real runs.
    pass1_importances = []
    for train_pos, _ in cv.split(t0_s.to_numpy(), t1_s.to_numpy()):
        model = fit_quantile(X_meta_full, mae_R, train_pos, 0.5)
    ...
    # A vs A+probability comparison (design section 9): run at q=0.75 (headline quantile),
    # fit with and without p_direction in the schema, keep it only if pinball loss is lower.
    X_meta_with_p = X_meta_full[feature_cols_meta]
    oof_pred_with_p = np.full(len(mae_R), np.nan)
    for _, (train_pos, test_pos) in enumerate(cv.split(t0_s.to_numpy(), t1_s.to_numpy())):
        model = fit_quantile(X_meta_with_p, mae_R, train_pos, 0.75)
        oof_pred_with_p[test_pos] = model.predict(X_meta_with_p.iloc[test_pos])
    has_pred_with_p = np.isfinite(oof_pred_with_p)
    yp_with_p, yt_with_p = oof_pred_with_p[has_pred_with_p], mae_R[has_pred_with_p]
    pinball_loss_with_p = pinball_loss(yt_with_p, yp_with_p, 0.75)

    cols_no_p = [c for c in feature_cols_meta if c != "p_direction"]
    X_meta_no_p = X_meta_full[cols_no_p]
    oof_pred_no_p = np.full(len(mae_R), np.nan)
    for _, (train_pos, test_pos) in enumerate(cv.split(t0_s.to_numpy(), t1_s.to_numpy())):
        model = fit_quantile(X_meta_no_p, mae_R, train_pos, 0.75)
        oof_pred_no_p[test_pos] = model.predict(X_meta_no_p.iloc[test_pos])
    has_pred_no_p = np.isfinite(oof_pred_no_p)
    yp_no_p, yt_no_p = oof_pred_no_p[has_pred_no_p], mae_R[has_pred_no_p]
    pinball_loss_no_p = pinball_loss(yt_no_p, yp_no_p, 0.75)

    keep_p_direction = pinball_loss_with_p < pinball_loss_no_p - 1e-4
    if keep_p_direction:
        feature_cols_final = feature_cols_meta
    else:
        feature_cols_final = cols_no_p

    X_meta = X_meta_full[feature_cols_final]
    ...
    schema = build_schema(f"mae_quantile_v3b_h{max_holding}", "2026-08-24", feature_cols_final)
    save_schema(schema, schemas_dir=schemas_dir if schemas_dir else SCHEMAS_DIR)
    entry = ModelRegistryEntry(
        model_id=f"mae_quantile_v3b_candidate_h{max_holding}", family="mae_quantile", algorithm="catboost_quantile",
        artifact_path=f"registry/mae_quantile_v3b_candidate_h{max_holding}.json",
        feature_schema_version=f"{schema.schema_id}__{schema.schema_version}",
        feature_cols=feature_cols_final,
        target_definition=(f"mae_R: max adverse excursion in R-multiples up to t1, direction_side/assumed_side "
                            f"sourced from {dir_oof['model_id']}'s own OOF prediction, max_holding={max_holding}"),
        training_config={"quantiles": QUANTILES, "n_splits": 6, "embargo_bars": max_holding * 2,
                          "direction_side_model_id": dir_oof["model_id"],
                          "pinball_loss_q0.75_with_p": pinball_loss_with_p,
                          "pinball_loss_q0.75_assumed_side_only": pinball_loss_no_p,
    ...
    print(f"[mae_quantile_v3b h={max_holding}] n_events={len(mae_R):,} "
          f"global_coverage={global_coverage} kept_p_direction={keep_p_direction} -> status={status}")
    return {"n_events": len(mae_R), "global_coverage": global_coverage,
            "per_regime_coverage": per_regime_coverage, "status": status}
