...
def market_closed():
    """True if the XM market is CLOSED (weekend / daily break).

    Two signals, OR'd:
      1. Wall-clock: XM XAUUSD closes Fri ~21:00 UTC, reopens Mon ~21:00 UTC.
         Sat/Sun are ALWAYS closed regardless of what MT5 returns.
      2. Ticker flag: xm_tick_state.json market_closed=true (stale tick /
         broker closed window detected by the ticker itself).

    The ticker only writes the flag when it gets a STALE tick back from MT5;
    on a weekend disconnect (t is None) the flag never lands, so without the
    wall-clock fallback the watchdog restart-churns the ticker all weekend
    and fires "stale bid none" notifications. During the closed window:
      - bid is legitimately None   → ticker_fresh() would report stale
      - engine journal is silent   → engine_fresh() would report frozen
    Both are FALSE ALARMS — freshness restarts must be skipped, only actual
    process death should trigger a restart. Returns True when we can't read
    the flag (fail-safe: prefer NOT restarting a healthy stack over restarting
    it into a duplicate-IPC churn on Monday reopen)."""
    try:
        now = time.gmtime()
        wd, hr = now.tm_wday, now.tm_hour  # Mon=0..Sun=6
        # XM GOLD.i# real hours (verified from live bars 2026-08-10):
        #   daily break 21:00-22:00 UTC; weekend Fri 21:00 -> Sun 22:00.
        # Closed = Fri>=21:00, Sat all day, Sun<22:00, daily 21-22h (Mon-Thu).
        # NOT Mon<21:00 (old bug: marked ~21h of OPEN market closed, which
        # made the engine skip signals all Monday morning).
        if (wd == 4 and hr >= 21) or wd == 5 or (wd == 6 and hr < 22) \
                or (21 <= hr < 22 and wd < 4):
            return True  # weekend / daily break
    except Exception:
        pass
    try:
        with open(TICKER_STATE) as f:
            st = json.load(f)
        return bool(st.get("market_closed"))
    except Exception:
        return True  # can't verify → assume closed, don't churn
...
def main():
    ...
    elif not closed and not engine_fresh():
        if restart_engine():
    ...
    #    Market closed: bid is legitimately None (no ticks) → skip the stale
    ...
    elif not closed and not ticker_fresh():
        if restart_ticker():
