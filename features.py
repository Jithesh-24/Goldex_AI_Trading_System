...
# Oct 28, Dec 9. NFP: first Friday each month @ 08:30 ET. CPI: ~mid-month
# (BLS schedule) @ 08:30 ET.
# v8.9c (2026-08-12): the REAL historical calendar replaces the old 2026-only
# feed — event_calendar.py has 387 events (2019-2026): FOMC statement+presser+
# minutes, NFP first-Friday, BLS-dated CPI, all DST-aware. The old feed
# generated ONLY 2026 (NFP+CPI hardcoded 12th) → training bars from 2019-2025
# saw flat-garbage min_to_event; FOMC (gold's dominant driver) wasn't even in
# it. Same _events() contract — zero changes downstream.
def _event_times():
    """List of (unix_ts, kind) for 2019-2026 real US macro events."""
    try:
        import event_calendar as _ec
        return [(_t, _k.lower()) for _t, _k in _ec.build()]
    except Exception:
        # Hard fallback: never crash the matrix build — emit 2026 NFP/CPI.
        ...
        for m in range(1, 13):
            ...
            ev.append((int(_dt(2026, m, fd, 8, 30, tzinfo=_tz.utc).timestamp()), "nfp"))
        ev.append((int(_dt(2026, 1, 12, 8, 30, tzinfo=_tz.utc).timestamp()), "cpi"))
        ev.sort()
        return ev
...
def add_position_state(df):
    """v8.8: position-state columns (day_pnl, streak, trades_today).
    Live-only signals — the engine injects REAL values at prediction time;
    historical matrix rows carry the fresh-day baseline 0.0 (semantically
    correct: no trades existed yet). Learned closed-loop state-pricing."""
    df = df.copy()
...
def add_position_state(df):
    """v8.8 (2026-08-11): position-state columns (live-only signals).

    day_pnl / streak / trades_today describe the ENGINE's current session
    state — they CANNOT be derived from a bar. Historical matrix rows get
    the 0.0 baseline (fresh day, no trades yet) which is semantically
    correct: the model learns the baseline market surface from 6yr, and
    the LIVE outcome rows (via merge_live_outcomes_appended) carry the real
    values so the model learns how P(win) shifts after losses. This is
    learned state-pricing, NOT a gate — trade frequency is untouched; the
    model re-prices the same setups given the state it is in.
    """
    df = df.copy()
    df["day_pnl"] = 0.0
    df["streak"] = 0.0
