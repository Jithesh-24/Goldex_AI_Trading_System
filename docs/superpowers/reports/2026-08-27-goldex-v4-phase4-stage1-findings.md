...
- ADDENDUM (flagged in whole-branch review): the GARCH(1,1) parameters (omega/alpha/beta) were fit by MLE over the full 300,000-row training partition, including the 240,000:300,000 slice later used as the OOS test region in Section 8's follow-up check. This is a mild look-ahead in *parameter estimation only* (the causal recursion producing `sigma2[t]` from `sigma2[t-1]`/`eps[t-1]` is unaffected and remains strictly causal). Since this bias can only inflate apparent OOS signal, and Section 8's result still came back at/below null, it makes that negative result more conservative, not less trustworthy — but a future re-run should refit GARCH on rows 0:240,000 only for a fully clean OOS check.
