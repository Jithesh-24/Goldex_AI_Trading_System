#!/usr/bin/env python3
"""
train_fixed2.py — BINARY + BALANCED + OOM-SAFE (max 10M train rows).
"""
import numpy as np
import lightgbm as lgb
import time, os, sys, json, gc
import warnings
warnings.filterwarnings('ignore')

BASE = os.path.dirname(os.path.abspath(__file__))

def main():
    t0 = time.time()
    
    meta = json.load(open(f"{BASE}/train_data_meta.json"))
    n_total = meta['n_rows']
    
    print(f"═══ FIXED v2: BINARY + BALANCED + OOM-SAFE ═══\n", flush=True)
    print(f"Total: {n_total:,} rows | max train: 10M rows\n", flush=True)
    
    X108 = np.memmap(f"{BASE}/train_data_x.npy", dtype=np.float32, mode='r', shape=(n_total, 108))
    y = np.memmap(f"{BASE}/train_data_y.npy", dtype=np.int8, mode='r', shape=(n_total,))
    
    unique, counts = np.unique(y, return_counts=True)
    print(f"Target: 0={counts[0]:,} ({counts[0]/n_total*100:.1f}%), 1={counts[1]:,} ({counts[1]/n_total*100:.1f}%)\n", flush=True)
    
    seeds = [42, 7, 2026]
    
    params = {
        "objective": "binary",
        "metric": "binary_logloss",
        "is_unbalance": True,
        "num_leaves": 63,
        "learning_rate": 0.05,
        "feature_fraction": 0.7,
        "bagging_fraction": 0.7,
        "bagging_freq": 5,
        "verbose": -1,
        "max_bin": 31,
        "num_threads": 4,
    }
    N_TREES = 200
    
    # OOM-SAFE: sliding window, max train=10M
    step = 5_000_000
    train_size = 10_000_000
    splits = []
    start = 0
    while start + train_size + step <= n_total:
        tr_start = start
        tr_end = start + train_size
        te_end = tr_end + step
        splits.append((tr_start, tr_end, te_end))
        start += step
    
    print(f"Walk-forward: {len(splits)} splits (train={train_size/1e6:.0f}M, test={step/1e6:.0f}M sliding)", flush=True)
    for i, (ts, te, tee) in enumerate(splits):
        print(f"  Split {i+1}: train [{ts/1e6:.0f}M-{te/1e6:.0f}M] test [{te/1e6:.0f}M-{tee/1e6:.0f}M]", flush=True)
    
    all_preds = np.zeros(n_total, dtype=np.float32)
    all_counts = np.zeros(n_total, dtype=np.int32)
    
    for si, seed in enumerate(seeds):
        print(f"\n── seed {seed} ({si+1}/3) ──", flush=True)
        
        for sp_idx, (tr_start, tr_end, te_end) in enumerate(splits):
            ds_tr = lgb.Dataset(X108[tr_start:tr_end], label=y[tr_start:tr_end])
            ds_te = lgb.Dataset(X108[tr_end:te_end], label=y[tr_end:te_end], reference=ds_tr)
            
            model = lgb.train(
                params, ds_tr,
                num_boost_round=N_TREES,
                valid_sets=[ds_te],
                callbacks=[lgb.log_evaluation(0)],
            )
            
            preds = model.predict(X108[tr_end:te_end])
            pred_labels = (preds > 0.5).astype(int)
            y_te = np.array(y[tr_end:te_end])
            acc = (pred_labels == y_te).mean()
            baseline = max(np.bincount(y_te)) / len(y_te)
            
            up_mask = y_te == 1
            down_mask = y_te == 0
            up_acc = (pred_labels[up_mask] == 1).mean() if up_mask.sum() > 0 else 0
            down_acc = (pred_labels[down_mask] == 0).mean() if down_mask.sum() > 0 else 0
            pred_up_pct = pred_labels.mean() * 100
            
            print(f"  Split {sp_idx+1}: acc={acc:.3f} (base={baseline:.3f}) "
                  f"up={up_acc:.3f} down={down_acc:.3f} pred_up={pred_up_pct:.1f}%", flush=True)
            
            all_preds[tr_end:te_end] += preds
            all_counts[tr_end:te_end] += 1
            
            del ds_tr, ds_te, model
            gc.collect()
    
    # Aggregate OOF (only for tested regions)
    tested = all_counts > 0
    oof_preds = (all_preds[tested] / all_counts[tested])
    oof_labels = (oof_preds > 0.5).astype(int)
    y_tested = np.array(y[tested])
    
    agg_acc = (oof_labels == y_tested).mean()
    baseline = max(np.bincount(y_tested)) / len(y_tested)
    up_mask = y_tested == 1
    down_mask = y_tested == 0
    up_acc = (oof_labels[up_mask] == 1).mean()
    down_acc = (oof_labels[down_mask] == 0).mean()
    pred_up_pct = oof_labels.mean() * 100
    
    print(f"\n═══ AGGREGATE OOF ═══", flush=True)
    print(f"  Test coverage: {tested.sum()/n_total*100:.1f}% of data", flush=True)
    print(f"  accuracy: {agg_acc:.3f} (baseline: {baseline:.3f})", flush=True)
    print(f"  up correct: {up_acc:.3f}", flush=True)
    print(f"  down correct: {down_acc:.3f}", flush=True)
    print(f"  predicted up: {pred_up_pct:.1f}% (actual: {y_tested.mean()*100:.1f}%)", flush=True)
    
    # ═══ FINAL MODELS on 130 features (last 10M rows) ═══
    print(f"\n═══ FINAL MODELS (130 features, last 10M) ═══\n", flush=True)
    
    n_tail = 10_000_000
    X130 = np.memmap(f"{BASE}/train_data_x_130.npy", dtype=np.float32, mode='r', shape=(n_tail, 130))
    y_tail = y[n_total - n_tail:n_total]
    
    os.makedirs(f"{BASE}/models", exist_ok=True)
    feats = meta['features']
    with open(f"{BASE}/models/features.json", "w") as f:
        json.dump(feats, f)
    
    for si, seed in enumerate(seeds):
        print(f"\n── Final model: seed {seed} ({si+1}/3) ──", flush=True)
        t1 = time.time()
        
        ds = lgb.Dataset(X130, label=y_tail)
        model = lgb.train(
            params, ds,
            num_boost_round=N_TREES,
            callbacks=[lgb.log_evaluation(0)],
        )
        
        path = f"{BASE}/models/gold_lgb_model_s{seed}.txt"
        model.save_model(path)
        
        preds = model.predict(X130)
        pred_labels = (preds > 0.5).astype(int)
        y_tail_np = np.array(y_tail)
        acc = (pred_labels == y_tail_np).mean()
        up_m = y_tail_np == 1
        dn_m = y_tail_np == 0
        up_a = (pred_labels[up_m] == 1).mean() if up_m.sum() > 0 else 0
        dn_a = (pred_labels[dn_m] == 0).mean() if dn_m.sum() > 0 else 0
        
        print(f"  ✅ {path} ({time.time()-t1:.0f}s) "
              f"acc={acc:.3f} up={up_a:.3f} down={dn_a:.3f}", flush=True)
        
        del model, ds
        gc.collect()
    
    ensemble = {
        "type": "binary_balanced",
        "version": "v9.2_balanced_oom_safe",
        "seeds": seeds,
        "num_class": 2,
        "is_unbalance": True,
        "oof_accuracy": float(agg_acc),
        "oof_up_acc": float(up_acc),
        "oof_down_acc": float(down_acc),
    }
    with open(f"{BASE}/models/ensemble.json", "w") as f:
        json.dump(ensemble, f)
    
    elapsed = time.time() - t0
    print(f"\n═══ ALL DONE ═══ ({elapsed:.0f}s = {elapsed/60:.1f}min)", flush=True)

if __name__ == '__main__':
    main()
