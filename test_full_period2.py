"""Correct verification: collect the ENTIRE first period (until the 2nd gap),
so forward paths (ext = 4×max_bars) are complete for all bars. Compare
target (exact) + mfe/mfa (1% rel) per geometry, order-agnostic."""
import sys
import numpy as np
import pandas as pd
...
for chunk in pd.read_csv(OUT, dtype={c: np.float32 for c in hdr if c != "time"},
    ...
    one = chunk[keep].copy()
    parts.append(one)
    # count gaps among kept bars
    kt = pd.to_datetime(one["time"].astype(str)).values
    if prev_ts is not None:
        d = (kt[0] - prev_ts) / np.timedelta64(1, "s")
        if d > 6 * 3600:
            gap_count += 1
    if len(one) > 1:
        d2 = np.diff(kt.astype("datetime64[s]").astype("int64")).astype(float)
        gap_count += int((d2 > 6 * 3600).sum())
    prev_time = times[-1] if len(times) else None
    prev_ts = kt[-1] if len(kt) else None
    if gap_count >= 2:
        break
...
p0 = periods[0]
n = len(p0)
print(f"FULL period 0: {n} bars | {p0['time'].iloc[0]} → {p0['time'].iloc[-1]} | "
      f"periods seen: {len(periods)}")
...
print(f"source rows: {len(src)} (expect {n*n_old})")
...
def close(a, b, rel=0.01):
    return abs(a - b) <= rel * max(abs(a), abs(b), 1e-6)
...
for bi in range(n):
    ...
    for gi in range(n_old):
        ...
        if not (float(sr["target"]) == tgt[gi, bi]
                and close(float(sr["mfe_atr"]), mfe[gi, bi])
                and close(float(sr["mfa_atr"]), mfa[gi, bi])):
            mm_m += 1
print(f"compared {tot} rows — target Δ: {mm_t} ({100*mm_t/tot:.2f}%) | "
      f"full Δ: {mm_m} ({100*mm_m/tot:.2f}%)")
print("PASS" if mm_t == 0 and mm_m == 0 else "CHECK REMAINING")
